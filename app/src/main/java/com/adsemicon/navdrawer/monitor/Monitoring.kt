...
import com.adsemicon.navdrawer.function.Function
...
class Monitoring {
    ...
    constructor() {
        ...
        for (i in 0 until MAX_CH_CNT) {
            ...
            if (i == DM_CH_IDX){
                if (com.adsemicon.navdrawer.Global.percentLog != null) {
                    com.adsemicon.navdrawer.Global.percentLog.headerText += " CH_DM(%)"
                }
            }else {
                //if (Global.touchLog != null && i < TCH_CH_CNT) {
                if (com.adsemicon.navdrawer.Global.touchLog != null) {
                    com.adsemicon.navdrawer.Global.touchLog.headerText += " CH${i + 1}"
                }
                if (com.adsemicon.navdrawer.Global.percentLog != null) {
                    com.adsemicon.navdrawer.Global.percentLog.headerText += " CH${i + 1}(%)"
                }
            }
        }
    }
    ...
    fun updateMonData(
        kind: com.adsemicon.navdrawer.PacketKind?,
        dataContents: ByteArray,
    ) {
        when (kind) {
            com.adsemicon.navdrawer.PacketKind.MonTouch -> setTouch(dataContents[0])
            com.adsemicon.navdrawer.PacketKind.MonPercent -> setPercent(dataContents)
            else -> { }  // Do nothing
        }
        com.adsemicon.navdrawer.Global.monitoring.hasNewData = true
    }
    ...
    private fun logMonData() {
        ...
        // Touch Log
        if (com.adsemicon.navdrawer.Global.touchLog.isEnabled) {
            logStr = ""
            for (i in 0 until TCH_CH_CNT) {
                synchronized(channels) {
            ...
            com.adsemicon.navdrawer.Global.touchLog.printMonData(logStr)
        }

        // Percent Log(Touch Log 와 데이터 저장 시점 일치 목적)
        if (com.adsemicon.navdrawer.Global.percentLog.isEnabled) {
            logStr = ""
            for (i in 0 until MAX_CH_CNT) {
                synchronized(channels) {
            ...
            com.adsemicon.navdrawer.Global.percentLog.printMonData(logStr)
        }
    }
