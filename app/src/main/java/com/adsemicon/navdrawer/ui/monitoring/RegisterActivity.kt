...
//    var timer : Timer? = null
    ...
    private fun displayAllRegisters() {
        ...
        for (r in com.adsemicon.navdrawer.Global.regCon.registers) {
        ...
        override fun afterTextChanged(p0: Editable?) {
            ...
            try {
                ...
                if (singleRegAddrValid) {
                    if (com.adsemicon.navdrawer.Global.regCon.hasRegister(uSingleAddr)) {
                        com.adsemicon.navdrawer.Global.regCon.setRegister(uSingleAddr, uSingleVal)
                        displayAllRegisters()
                    }
                }
            }
    ...
    private val listenerReadSingle = View.OnClickListener {
        ...
        if (!singleRegAddrValid) {
            ...
            Packet.send(com.adsemicon.navdrawer.Global.outStream,
                com.adsemicon.navdrawer.PacketKind.RegSingleRead,
    ...
    private val listenerWriteSingle = View.OnClickListener {
        ...
        if (!singleRegAddrValid) {
            ...
            Packet.send(com.adsemicon.navdrawer.Global.outStream, com.adsemicon.navdrawer.PacketKind.RegSingleWrite, ba) // Send packet
            etSingleAddr.clearFocus()
            btnReadSingle.requestFocus()
    ...
    private val listenerReadAll = View.OnClickListener {
        try {
            ...
            Packet.send(com.adsemicon.navdrawer.Global.outStream,
                com.adsemicon.navdrawer.PacketKind.RegSingleRead,
                com.adsemicon.navdrawer.Global.regCon.registers[regIndex].addr.toByte()) // Send packet
            pbRegister.progress = 0
        } catch (ex: Exception) {
    ...
    private val listenerWriteAll = View.OnClickListener {
        try {
            ...
            var ba = ByteArray(2)
            ba[0] = com.adsemicon.navdrawer.Global.regCon.registers[regIndex].addr.toByte()  // Address
            ba[1] = com.adsemicon.navdrawer.Global.regCon.registers[regIndex].value.toByte()  // Value
        ...
        override fun onItemClick(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
            when (p0?.id) {
                R.id.gridAllRegisters -> {
                    val addr = com.adsemicon.navdrawer.Global.regCon.registers[p2].addr
                    val value = com.adsemicon.navdrawer.Global.regCon.registers[p2].value
...
//                }
                ...
                if (!qEmpty) {
                    try {
                        ...
                        when (packet.kind) {
                            com.adsemicon.navdrawer.PacketKind.RegSingleRead -> packetProcRegRead(packet)
                            com.adsemicon.navdrawer.PacketKind.RegSingleWrite -> {
                                packetProcRegWrite()
                                //Thread.sleep(1)
                            }
                            else -> {}    // Do nothing
        ...
        private fun packetProcRegRead(packet: RPacket) {
            val regSize = com.adsemicon.navdrawer.Global.regCon.registers.size
            val uAddr = packet.dataList[0].toUByte()
            val uVal = packet.dataList[1].toUByte()

            if (com.adsemicon.navdrawer.Global.regCon.hasRegister(uAddr)) {
                com.adsemicon.navdrawer.Global.regCon.setRegister(uAddr, uVal)
            }
        ...
        private fun packetProcRegWrite() {
            ...
            if (rwAll) {
                if (++regIndex == regSize) {
                    ...
                    ba[0] = com.adsemicon.navdrawer.Global.regCon.registers[regIndex].addr.toByte()
                    ba[1] = com.adsemicon.navdrawer.Global.regCon.registers[regIndex].value.toByte()
                    Packet.send(com.adsemicon.navdrawer.Global.outStream, com.adsemicon.navdrawer.PacketKind.RegSingleWrite, ba) // Send packet
